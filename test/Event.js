...
describe('Event', (accounts) => {
  ...
  describe('Buy Ticket from User', async () => {
    ...
    it('ticket should belong to buyer 2', async () => {
      ...
      await hardhatEvent.connect(buyer1).setTicketForSale(ticketID, ticketPrice);
      await hardhatEvent.connect(buyer1).approveAsBuyer(buyer2.address, ticketID);
      await hardhatEvent.connect(buyer2).buyTicketFromUser(ticketID, { value: ticketPrice });
  ...
  describe('setStage transitions', async () => {
    it('rejects invalid transitions', async () => {
      const { hardhatEvent, owner } = await loadFixture(deployEventFixture);

      await expect(hardhatEvent.connect(owner).setStage(2)).to.be.reverted;
      await expect(hardhatEvent.connect(owner).setStage(4)).to.be.reverted;
    });
    ...
    it('allows valid forward transitions', async () => {
      ...
      expect(await hardhatEvent.stage()).to.equal(4);
    });

    it('processes cancellation only once', async () => {
      const { hardhatEvent, owner, buyer1 } = await loadFixture(deployEventFixtureSetStage);

      await hardhatEvent.connect(buyer1).buyTicket({ value: _price });
      await hardhatEvent.connect(owner).setStage(3);
      expect(await hardhatEvent.balances(owner.address)).to.equal(0);

      await expect(hardhatEvent.connect(owner).setStage(3)).to.be.reverted;
    });
  })
})
