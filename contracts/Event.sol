...
contract Event is ERC721 {
    ...
    function setStage(Stages _stage) public onlyOwner returns (Stages) {
        require(_isValidStageTransition(stage, _stage), "");

        stage = _stage;
        if (_stage == Stages.Cancelled && !isCancelled) {
            isCancelled = true;
    ...
    function _isValidStageTransition(Stages currentStage, Stages nextStage) internal pure returns (bool) {
        if (currentStage == Stages.Closed) {
            return false;
        }
        if (currentStage == Stages.Prep) {
            return nextStage == Stages.Active || nextStage == Stages.Cancelled;
        }
        if (currentStage == Stages.Active) {
            return nextStage == Stages.CheckinOpen || nextStage == Stages.Closed || nextStage == Stages.Cancelled;
        }
        if (currentStage == Stages.CheckinOpen) {
            return nextStage == Stages.Closed || nextStage == Stages.Cancelled;
        }
        if (currentStage == Stages.Cancelled) {
            return nextStage == Stages.Closed;
        }
        return false;
    }
